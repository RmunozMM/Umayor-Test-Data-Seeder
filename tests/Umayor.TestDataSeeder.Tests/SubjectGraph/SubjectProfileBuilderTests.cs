...
namespace Umayor.TestDataSeeder.Tests.SubjectGraph
{
    ...
    public class SubjectProfileBuilderTests
    {
        ...
        /// AttributeWritabilityRules.GetWritableAttributes escribía ambos. Primer intento real
        /// (revertido) excluyó "objectid"; eso cambió el error a "The ObjectID is missing.",
        /// confirmando en vivo que en este tenant la vía válida es ObjectTypeCode+ObjectId, no
        /// activityid. "activityid" debe quedar excluido para esta tabla (dejando "objectid");
        /// ninguna otra tabla del mapa debe tener exclusiones.
        /// </summary>
        [Fact]
        public async Task BuildAsync_ActivityMimeAttachment_ExcludesActivityId_OtherTablesUnaffected()
        {
            var contactId = Guid.NewGuid();
            var contact = new DataRecord("contact", contactId);
            contact.Attributes["wit_rut"] = "12345678";
            ...
            Assert.Equal(new[] { "activityid" }, attachment.ExcludedAttributes);
